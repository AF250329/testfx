...
function Locate-VsInstallPath($hasVsixExtension = "false") {
  ...
  try {
    if ($Official -or $DisallowPrereleaseMSBuild) {
      ...
      $vsInstallPath = & $vswhere -latest -products * -requires $requiredPackageIds -property installationPath | Select-Object -First 1
    }
    else {
      ...
      $vsInstallPath = & $vswhere -latest -prerelease -products * -requires $requiredPackageIds -property installationPath | Select-Object -First 1
    }
  }
